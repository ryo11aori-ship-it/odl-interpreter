package main
import("fmt";"os";"strings";"strconv";"math";"image";"image/color";"image/png")
...
for i:=0;i<maxI;i++{
nc:=Cell{r,i};if nc!=c&&Dist(c,nc)<1.5{res=append(res,nc)}
}
}
return res
}
func GetDir(state string)(float64,float64){
if state=="GU"{return 0,1};if state=="GD"{return 0,-1};if state=="GL"{return -1,0};if state=="GR"{return 1,0};return 0,0
}
func Field(c Cell,buf map[Cell][]string,s string,maxR int)string{
cx,cy:=c.XY();dx,dy:=GetDir(s);px,py:=0.0,0.0;found:=false
for r:=c.R-2;r<=c.R+2;r++{
if r<0||r>maxR{continue}
maxI:=8*r;if r==0{maxI=1}
for i:=0;i<maxI;i++{
nc:=Cell{r,i}
hasP:=false;for _,st:=range buf[nc]{if st=="P"{hasP=true}}
if hasP{
nx,ny:=nc.XY();vx,vy:=nx-cx,ny-cy
if vx*dx+vy*dy>=-0.1{
...
if !found{return s}
dirs:=[]string{"GU","GD","GL","GR"};dxs:=[]float64{0,0,-1,1};dys:=[]float64{1,-1,0,0};md:=-999.0;best:=s
for j:=0;j<4;j++{dot:=px*dxs[j]+py*dys[j];if dot>md{md=dot;best=dirs[j]}}
return best
}
func Resolve(states []string)string{
if len(states)==0{return ""};if len(states)==1{return states[0]}
c:=make(map[string]int);for _,s:=range states{c[s]++}
if c["R"]>0&&c["B"]>0&&c["Y"]>0{return "R"}
if c["R"]>0&&c["B"]>0&&c["Y"]==0{return "Y"}
if c["R"]>0&&c["Y"]>0{return "B"};if c["B"]>0&&c["Y"]>0{return "R"}
if c["R"]>0{return "R"};if c["B"]>0{return "B"}
prio:=map[string]int{"P":90,"Y":80,"O":70,"GU":60,"GD":60,"GL":60,"GR":60,"R":50,"B":50,"C":40,"K":30,"Br":20,"W":10};b:="";mp:=0
...
p:=strings.Split(line,":");c:=strings.Split(p[0][1:],",");r,_:=strconv.Atoi(c[0]);i,_:=strconv.Atoi(c[1])
if r>maxR{maxR=r};buf[Cell{r,i}]=append(buf[Cell{r,i}],strings.TrimSpace(p[1]))
}
}
for c,states:=range buf{grid[c]=Resolve(states)}
Draw(grid,maxR,0)
...
dx,dy:=GetDir(s)
if dx!=0||dy!=0{
var best Cell;maxDot:=-999.0;cx,cy:=c.XY()
for _,n:=range Neighbors(c,maxR+1){
nx,ny:=n.XY();vx,vy:=nx-cx,ny-cy;norm:=math.Sqrt(vx*vx+vy*vy)
if norm>0{vx/=norm;vy/=norm};dot:=vx*dx+vy*dy;if dot>maxDot{maxDot=dot;best=n}
}
if maxDot>-999.0{nextBuf[best]=append(nextBuf[best],s)}else{nextBuf[c]=append(nextBuf[c],s)}
}else{nextBuf[c]=append(nextBuf[c],s)}
}
for c,states:=range nextBuf{
for i,s:=range states{if strings.HasPrefix(s,"G"){states[i]=Field(c,nextBuf,s,maxR+1)}}
}
